...
const TYPE_ACCENT: Record<string, string> = {
  ...
  title_up: "#eab308",
  anniversary: "#ec4899",
  alliance_boss: "#dc2626",
  codex_complete: "#0ea5e9",
  custom: "#8b5cf6",
};

function relativeTime(s?: string): string {
  if (!s) return "";
  const ts = new Date(s.replace(/\//g, "-").split(" ")[0]).getTime();
  ...
  if (day < 30) return `${Math.floor(day / 7)} 週前`;
...
function MomentCard({ m, idx, onLike }: { m: any; idx: number; onLike: () => void }) {
  const [burst, setBurst] = useState(false);
  const accent = TYPE_ACCENT[m.type] ?? "#8b5cf6";
  ...
  return (
    <div
      ...
      } ${m.type === "ssr_card" ? "shine-ssr" : ""} relative overflow-hidden`}
      style={{
    ...
    >
      <div className="flex items-start gap-4">
        <div className="text-5xl shrink-0 drop-shadow-md">{m.emoji}</div>
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-2 text-xs text-empire-mute flex items-center gap-2 flex-wrap">
            ...
            <span>·</span>
            <span title={m.createdAt}>{relativeTime(m.createdAt)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 flex items-center gap-2">
        <button
          onClick={handleLike}
          className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-semibold transition active:scale-90 relative ${
        ...
        >
          ...
          {burst && (
            <span
              className="absolute -top-2 -right-1 text-2xl pointer-events-none"
              style={{ animation: "heart-burst 0.6s ease-out forwards" }}
            >
              ❤️
            </span>
          )}
